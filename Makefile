...
bake-and-test-deploy: ## For quick publishing to cookiecutter-ds-example to test GH Actions
	@rm -rf cookiecutter-ds-example || true
	@uv run cookiecutter --no-input . --overwrite-if-exists \
		author="Joe Black" \
	...
	@cd cookiecutter-ds-example; uv sync && \
		...
		git remote add origin git@github.com:joeblackwaslike/cookiecutter-ds-example.git && \
